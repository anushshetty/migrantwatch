<? session_start();
   ...
   if($_REQUEST['save_changes']) {
        ...
        if( $location = $_POST['location'] ) { $url .= "&location=" . $location; } else { $location = ''; }
        ...
        $sql = "UPDATE migwatch_species_watchlist SET url='$url', location='$location', season='$season', state='$state', stype='$type',user='$user' where id='$id'";
        mysql_query($sql);
	?><script>parent.page_refresh();
...
<form  enctype="multipart/form-data" action="editspecieswatchlist.php" method="post">
<table>
<tr>
   <td style=''>season</td>
   <td>
