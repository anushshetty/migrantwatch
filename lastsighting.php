...
function validate_<? echo $type; ?>() {
		...
		var obstart = document.frm_last.obstart.value;
		var tbl = $('#catTable_<? echo $type; ?>');
		var lastRow = $('#catTable_<? echo $type; ?> tr').length - 1;
		

		if(document.frm_last.location.value == "") { 
			//alert("Please select a valid location for the Observations.");
                  jAlert('Please select a valid location for the Observations.', 'Location');                       
						return false;
		}

      		if((obstart == "") || (obstart == obstartHint)) {
			jAlert("Please enter the date when you stopped looking for birds at this location \n(in the current migration season)");
			return false;
      		} 
...
$().ready(function() {
    ...
    $("#addCurrentLocation_<? echo $type; ?>").result(function(event, data, formatted) {
        ...
        $('#location_<? echo $type; ?>').val(data[1]);
         if(data[1] == '0' ) { document.getElementById('addCurrentLocation_<? echo $type; ?>').disabled = true; $('#locDetails_<? echo $type; ?>').hide(); $('#dialog_<? echo $type; ?>').jqm().jqmShow({
          overlay: 70,
          autofire: true
        });  } else { 
...
#lastsighting input { font-size:11px;border: solid 1px #777777; padding:5px }

#lastsighting td { font-size:13px; }
...
<div id="gen_form">
	...
	<form id="lastsighting" method='post'  name='frm_last' action='savesighting.php'>
...
<input size="30" type=text id="other_name_<? echo $type; ?>" name="other_name" value="Name" style="" onKeyUp="update_others_<? echo $type; ?>();" onKeyDown="update_others_<? echo $type; ?>();">
			...
			<tr>
					<td><a href="#" style='font-size:14px;' class="jqModal">Add a new Location</a></td>
         			...
         			<div class="jqmWindow" id="dialog_<? echo $type; ?>" style="width:800px">
...
JS;
	...
	<tr>
        ...
        <input type="hidden" name="last" value="1"/>
			<input type="hidden" name="cmd" value="lastsighting" id='cmd'/>
