...
MAILTXT;
				...
				header("Location: index.php?cmd=registered");
				exit();
			}else
	...
	if($result){
		if (mysql_num_rows($result) > 0){
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)){
				...
				mysql_query($tempsql);
			       if( $_POST['remember'] == '1'){
                	       	   setcookie("cookname", $_SESSION['username'], time()+3600000, $src_base);
        			   setcookie("cookpass", $_SESSION['password'], time()+3600000, $src_base);
         			}
