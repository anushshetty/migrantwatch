<?php session_start();
	...
	if ($_GET['last'] == 1 || $_POST['last'] == 1 || $_SESSION['trackspecies']['last'] == 1) {
		$last = 1;
		$start = 'end';
		$look = 'did you look';
		$sighting = 'Last';
	...
	if (!empty($_GET['loc_id']) || (($_POST['cmd'] == 'loc_change') && !empty($_POST['location']))) {
		...
		if (isValidLocation($location_id,$connect)) {
			$sql = "SELECT obs_start,frequency FROM migwatch_l1 WHERE location_id='$location_id' " .
				   "AND user_id = '$user_id' AND obs_start >'$currentSeason-06-30' AND obs_type = '$sighting' AND deleted = '0' ORDER BY sighting_date DESC LIMIT 0,1";
			$res = mysql_query($sql);
...
<script language="JavaScript" type="text/javascript">
	...
	function addLocation(){
		...
		document.frm_level1.action="process_cuckoo.php";
		document.frm_level1.submit();
	}

	function stateChanged() {
		document.frm_level1.cmd.value = "statechanged";
		document.frm_level1.action = "trackcuckoo.php";
		document.frm_level1.submit();
	}
	...
	function validate() {
		...
		for(var i = 1; i <= lastRow; i++) {
			...
			if ($('#species_' + i).val() != '' && $('#obdate_' + i).val() != '') {
				...
				var sighttype = $('#sighting_type_' + i).val();
				
				if (sighttype == '') {
					alert('Please select the Sighting type.');
	...
	function setLocationData() {
		...
		document.forms[0].action = 'trackcuckoo.php';
...
<form method='post' name='frm_level1' action='process_cuckoo.php'>
...
<table border=0 width="770" cellpadding=2 cellspacing=1>
    ...
    ?>
	<tr>
		<td align=center bgcolor="#6D79C8" colspan=2><b>P I E D&nbsp;&nbsp;&nbsp;C U C K O O&nbsp;&nbsp;&nbsp;C A M P A I G N</b></td>
	</tr>
	<tr>
		<td align=left colspan=2><img align=top src="jacobinlogo_small.jpg" style="float:left" hspace=4>Please use this form for submitting First Sighting dates for Pied Cuckoos.
                To submit First Sighting dates for other migrants <a href="trackspecies.php">click here</a>. If you see Pied Cuckoos several times through the year, please do submit these sightings using the <a href="gensightings.php">General Sightings</a> form.</td>
	</tr>
	<tr>
		<td bgcolor="#9baccb" colspan=2><b>Reported By</b></td>
	</tr>
	<tr>
    ...
    ?>
	<tr>
		<td valign=top bgcolor="#9baccb" colspan=2>
    ...
    ?>
	<tr>
		<td bgcolor="#9baccb" colspan=2><b>Observation Details</b></td>
	</tr>
	<tr>
		<td align=right>When did you <?php print(strtoupper($start)); ?> looking for Pied Cuckoos at this location? (in this calendar year, ie 1 Jan to 31 Dec) </td>
		<td>
	...
	<tr>
		<td align=right>How often <?php print($look); ?> for Pied Cuckoos at this location?</td>
		<td>
			<select name="often">
				<option value="" >Select</option>
...
<table id='catTable'>
	...
	<tr>
		...
		<td width='180px'><?php print($sighting); ?> Sighting Date*<br>(dd-mm-yyyy)</td>
		<td>Number</td>
	...
	<tr id='cloneme'>
		...
		<td><select id= 'sighting_type_<?php echo $i ?>' name='sighting_type[]'>
				<option value=''>-- Select --</option>
				<option value='saw' <?php if(isset($lastData['sighting_type'][$j]) && $lastData['sighting_type'][$j] == 'saw') echo 'selected' ?>>Saw</option>
				<option value='heard' <?php if(isset($lastData['sighting_type'][$j]) && $lastData['sighting_type'][$j] == 'heard') echo 'selected' ?>>Heard</option>
				<option value='saw_heard' <?php if(isset($lastData['sighting_type'][$j]) && $lastData['sighting_type'][$j] == 'saw_heard') echo 'selected' ?>>Saw & Heard</option>
			</select>
		     </td>
		<td><select id= 'flight_dir_<?php echo $i ?>' name='flight_dir[]'>
	     <option value=''>-- Select --</option>
	    ...
	    <option value='E-W' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'E-W') echo 'selected' ?>>E-W</option>
           <option value='NE-SW' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'NE-SW') echo 'selected' ?>>NE-SW</option>         <option value='N-S' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'N-S') echo 'selected' ?>>N-S</option>
	...
	<tr> <td><P> </td> <td><P> </td> </tr>
        <tr><td align=right><input type="checkbox" name="resident_check" value=1></td><td colspan=6><b>Please check this box if you have seen Pied Cuckoos at your location between Dec and Mar in any year.</b></td></tr>
        <tr><td colspan=7><P>Note that fields marked with an asterisk (*) are mandatory.</td></tr>
	<?php
			if ($i > 1) {
				echo <<<JS
...
<div style="margin-top:5px;">
	&nbsp;&nbsp;<input type='submit' style="height:3em;" value= "Submit Data"  class='buttonstyle' onclick="return validate();"> 
	&nbsp;&nbsp;<input type=button style="height:3em;" value="Cancel and return to the Pied Cuckoo Campaign page" class=buttonstyle onclick="javascript:window.location.href='piedcuckoo.php';" />
</div>
<?php
    } else {
        print("</table>");
...
<?php
	...
	if (!empty($lastData['location'])) {
		// Get the current season details. 21 is the species_id for Pied Cuckoo
		list($seasonStart,$seasonEnd) = getCurrentSeason(true);

		$sql = "SELECT l1.obs_start,l1.number,l1.notes,s.common_name, l.location_name, l.city, st.state, l1.sighting_date, l1.species_id, l1.location_id, l1.sighting_type,";
		...
		$sql.= "WHERE l1.user_id = '".$user_id."' AND l1.species_id = '21' AND l1.deleted = '0' AND ";
...
<table border=0 width="770" cellpadding=2 cellspacing=1>
	<tr>
		<td bgcolor="dedede" colspan=2><b><?php if ($total_sightings > 10) { echo '10 '; } echo strtolower($sighting) ?> sighting submitted by you in the current season at this location.</b></td>
	</tr>
		...
		<table width=98% cellpadding=3 cellspacing=0 style="border-width:1px;border-style:solid;border-color:#dedede">
			...
			if($result) {
				...
				while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
					...
					print "<td>"; echo (empty($row{'sighting_type'})) ?  '--' : $row{'sighting_type'}; print "</td>";
					$id1 = $row['id'];
					$query = "select resident_check from piedcuckoo_migwatch where sighting_id = '$id1'";
					$query = mysql_query($query);
					$data = mysql_fetch_assoc($query);
                                        if($data['resident_check'] == '1')
                                           {
                                              $data['resident_check'] = 'Yes'; 
                                            } 
                                        else if($data['resident_check'] == '0')
                                            { 
                                             $data['resident_check'] = 'No'; 
                                            }
					print "<td>"; echo (empty($data['resident_check'])) ?  '--' : $data['resident_check']; print "</td>";
					print "</tr>";
					$i++;
					$j++;
				}
