...
function appendNewInput() {	 
	...
	var iteration = lastRow - 1;
 	var newElement ="<tr id='tr_" + iteration + "' class='sprow'>" + "<td>"
		...
		+ "</td><td><small><a class='sprow_del' id='sprow_del_" + iteration + "' title='Click here to delete this report'  href='#x'>Delete</a></small></td>"
		+ "</tr>";

		
	$('#catTable').append(newElement);
	
	$('.sprow_del').click( function() {
   	  var sprow_id  = $(this).attr('id');
          sprow_id = sprow_id.replace(/^sprow_del_/, 'tr_');
          jConfirm('Are you sure you want to delete this report ?', '',function(r) {
          if(r==true) {
	    $('#' + sprow_id +' td *').val('');
	    $('#' + sprow_id).fadeOut("slow");
	    return false;
          }
        });
	});
...
function new_info(loc_id,from_form) {
 ...
 $().ready(function() {
$("#sighting_submit").click(function() {
     	var now = new Date(); 
 		...
 		if ( ( i == 1 ) ||  (  (( $('#species_' + i).val() != '' ) && ( $('#species_' + i).val() != speciesHintText )  ) || ( ( $('#obdate_' + i).val() != '' ) && ( $('#obdate_' + i).val() != dateHintText ) ) )) {
			...
			if( sigtype == 'last') {                       
				...
				var num = document.getElementById('number_' + i).value;
                                ...
                                var sighting_type = $('#sighting_type_' + i).val();
