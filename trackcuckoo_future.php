...
<script language="JavaScript" type="text/javascript">
	...
	function stateChanged() {
		...
		document.frm_level1.action = "trackcuckoo2.php";
		document.frm_level1.submit();
	}
	...
	function validate() {
		...
		for(var i = 1; i <= lastRow; i++) {
			...
			if ($('#species_' + i).val() != '' && $('#obdate_' + i).val() != '') {
				...
				var flight_dir = $('#flight_dir_' + i).val();
				
				if (flight_dir == '') {
					alert('Please select the Flight direction.');
					$('#flight_dir_' + i).focus();
					return false;
				}
			}
	...
	function setLocationData() {
		...
		document.forms[0].action = 'trackcuckoo2.php';
...
if ($_GET['error'] == 1) {
	$error = 1;
	$lastData = $_SESSION['trackspecies'];
	$rowCount = count($lastData['obdate']);
...
<table border=0 width="770" cellpadding=2 cellspacing=1>
	...
	<tr>
		<td align=right>When did you <?php print(strtoupper($start)); ?> looking for birds at this location? (in the current migration season, ie 1 July to 30 June) </td>
		<td>
...
<table id='catTable'>
	...
	<tr id='cloneme'>
		...
		<td><select id= 'flight_dir_<?php echo $i ?>' name='flight_dir[]'>
				<option value=''>-- SELECT --</option>
				<option value='N-S' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'N-S') echo 'selected' ?>>Saw</option>
				<option value='N-NW' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'N-NW') echo 'selected' ?>>Heard</option>
				<option value='N-NS' <?php if(isset($lastData['flight_dir'][$j]) && $lastData['flight_dir'][$j] == 'N-NS') echo 'selected' ?>>Saw & Heard</option>
			</select>
		     </td>

	</tr>
        <tr><td><input type="checkbox" name="resident_check" value=1></td><td colspan=6><b>Check the box if you have seen this bird between December and March months</b></td></tr>
	<?php
			if ($i > 1) {
				echo <<<JS
