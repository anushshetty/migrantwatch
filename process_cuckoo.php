<?php
	...
	if($_POST['resident_check']) { $_POST['resident_check'] = '1'; } else { $_POST['resident_check'] = '0'; }
	...
	$filtered_data['flight_dir'] = $_POST['flight_dir'];
        ...
        /*if($filtered_data['resident_check'] == '1'){
	  $filtered_data['resident_check'] = "Yes";
	}else { $filtered_data['resident_check'] = "No"; }
	*/
        ...
        $sql1 = "SELECT * FROM piedcuckoo_migwatch WHERE sighting_id = '".$filtered_data['id']."'";
        $res1 = mysql_query($sql1,$connect2);
        $data1 = mysql_fetch_assoc($res1);
        //$data1['resident_check'];
        $oldData = array(
                            'id'          => $data['id'],
			    ...
			    'sighting_type' => $data['sighting_type'],
			    'flight_dir' => $data['flight_dir'],
			    'resident_check' => $data1['resident_check']
                        );
...
//print_r($oldData);print("<br>");print_r($newData);exit;
        ...
        $sql = "UPDATE migwatch_l1 SET " .
	        ...
	        "flight_dir = '".$filtered_data['flight_dir']."'$updated_by WHERE id = '$data[id]'";
        $success = mysql_query($sql,$connect);

        $sql1 = "UPDATE piedcuckoo_migwatch SET resident_check='".$filtered_data['resident_check']."'WHERE sighting_id = '$data[id]'";
	...
	$flight_dir = array();
        for($i=0; $i<count($_POST['obdate']); $i++) {
		    ...
		    $sighting_type[] = getEscaped($_POST['sighting_type'][$i]);
        ...
        if($result) {
            for($i=0; $i<count($obdate); $i++) {
                ...
                if($sightingdate != "") {
                    ...
                    if($result1) {
                        if(mysql_num_rows($result1) == 0) {
                            ...
                            $sql = "INSERT INTO migwatch_l1(user_id,location_id,species_id,sighting_date,
				obs_start,frequency,notes,user_friend,obs_type,number,accuracy,entry_notes,
					sighting_type,flight_dir)";
			    ...
			    $new_POST['resident_check'][] = $resident_check[$i];
                        }
                    }
                }
            }
        ...
        if ($insertfailed || $duplicate) {
            ...
            $_SESSION['trackcuckoo'] = $new_POST;
            $_SESSION['trackcuckoo']['duplicate'] = $message;
            $_SESSION['trackcuckoo']['duplicate_species'] = $duplicate_species;
            $_SESSION['trackcuckoo']['errors'] = $error;
            header('Location: trackcuckoo.php?error=1');
            exit;
        } else {
            ...
            if (true) {
                header("Location: piedcuckoo.php?cmd=insertsuccess");
                foreach($_POST as $key=>$value) {
                    unset($_SESSION[$key]);
                }
                exit();
