...
<script language="JavaScript" type="text/javascript">
	...
	var speciesHintText = 'Type a name here';
        var dateHintText = 'Select';
        var numberHintText = 'In numerals';
        var notesHintText = 'Enter notes here';
...
function validate() {
		...
		for(var i = 1; i <= lastRow; i++) {
			...
			if ( ($('#obdate_<? echo $type; ?>_' + i).val() != '' || $('#obdate_<? echo $type; ?>_' + i).val() != dateHintText) ||  (i == 1)) {
			...
			if (( ($('#species_<? echo $type; ?>_' + i).val() != '' || $('#species_<? echo $type; ?>_' + i).val() != speciesHintText)) || (i == 1)) {
				if (($('#obdate_<? echo $type; ?>_' + i).val() == '') || $('#obdate_<? echo $type; ?>_' + i).val() == dateHintText)) {
					...
					var obdate = $('#obdate_<? echo $type; ?>_' + i).val();
					var ob_type = $('#ob_type_<? echo $type; ?>' + i).val();
...
if ($('#species_<? echo $type; ?>_' + i).val() != '' && $('#obdate_<? echo $type; ?>_' + i).val() != '') {
				...
				if (!isWhitespace(num) && isNaN(num)) {
					alert('Please enter numerals under \'Number\'');
...
<table id='catTable_<? echo $type; ?>' style="width:700px;margin-left:20px;font-size:11px" >
	...
	?>
	 <tr id='cloneme'>
		...
		<td>
 <input type='text' id='species_<? echo $type; ?>_<?php echo $i; ?>' style='font-size:11px' name='species[]' size='20' value='<? echo $speciesHintText; ?>'></td>
		 <td>
	...
	<td><input type="text" style='font-size:11px' id='number_<? echo $type; ?>_<?php echo $i; ?>' name='number[]' size="8" value='<? echo $numberHintText; ?>'></td>
		<td>
...
<tr><td><span><a href='#x' onclick='appendNewInput_<? echo $type; ?>();'>Add another species</a></td></tr>
