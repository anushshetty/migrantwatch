...
function get_recent_news() {
	$connectw=mysql_connect("localhost", "ncf","ncfproject");
        $wordpress_db=mysql_select_db("migwatch",$connectw );
        $sql="SELECT ID,post_title,post_excerpt  FROM wp_posts, wp_term_relationships, wp_term_taxonomy WHERE wp_term_relationships.object_id = wp_posts.id AND post_status= 'publish' and post_type = 'post' AND wp_term_taxonomy.term_taxonomy_id = wp_term_relationships.term_taxonomy_id AND wp_term_taxonomy.taxonomy ='category' AND wp_term_taxonomy.term_id = 56 ORDER BY id DESC LIMIT 1";

        $query_result = mysql_query($query);
        mysql_close();

         $recent_news['id'] = mysql_result($query_result, 0, "ID");
         $recent_news['title'] = mysql_result($query_result, 0, "post_title");
	 $recent_news['excerpt'] = mysql_result($query_result, 0, "post_excerpt");
         return $recent_news;
}
...
function get_latest_blog() {
        $connectw=mysql_connect("localhost", "ncf","ncfproject");
	$wordpress_db=mysql_select_db("migwatch",$connectw );
        $sql="Select ID, post_title FROM wp_posts WHERE post_type='post' AND post_status='publish' ORDER BY id DESC LIMIT 5";
