...
$sql = "SELECT l1.obs_start,l1.number,l1.notes, u.user_name,s.common_name, l.location_name, l.city, st.state, l1.sighting_date, l1.species_id, l1.location_id, ";
		...
		$sql.= "INNER JOIN migwatch_users u ON l1.user_id = u.user_id ";
                $sql.= "WHERE l1.deleted = '0' AND u.user_id != '$user_id' AND  ";
                $sql.= " l1.location_id = '$location_id' AND l1.valid=1 and l1.deleted='0' ";
		if( $species_id ) { $sql.= " AND l1.species_id='$species_id'"; }
                /* $sql.= " AND l1.obs_start BETWEEN '$seasonStart-07-01' AND '$seasonEnd-06-30'";
		$sql.= " AND l1.obs_start <> '1999-11-30' AND l1.obs_start <> '0000-00-00' "; */
                $sort = 'DESC';
...
<? } else {  
   ...
   echo "<div class='notice'>There are no previous " . ucfirst($sighting_type) . " sighting from this location</div>";
