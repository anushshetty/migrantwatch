...
<input type="text" name="location" id="location" value="" />
    ...
    <tr bgcolor="#efefef">
        <td>State / UT</td>
        <td><div id="details_state"></div><div id="obstart_test"></div></td> <div id="location2"></div>
    </tr>
...
<?php
	...
	if ($location_get) {
		...
		$sql.= " l1.location_id = '$location_get' AND obs_type = '".strtolower($sighting)."' ";
...
<script>
 ...
 $().ready(function() {
    ...
    $("#addCurrentLocation").result(function(event, data, formatted) {
     ...
     $sql = "select test from demo_loc where id=1";
     $result = mysql_query($sql);
      $total_rows = mysql_num_rows($result);
   
      ?> 
       
      var table = "<a href=\"querywindow.html?id=" + loc_data + "\" class=\"thickbox\" title=\"Detailed information of selected issue\">View Previous</a>";
