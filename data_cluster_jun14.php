  		...
  		GEvent.addListener(marker, "click", function() {
			marker.openExtInfoWindow(
			      map,
              		      ...
              		      "<div>Loading...</div>",
              		      {
				ajaxUrl: "getcontent.php?content=" + encodeURIComponent(html_content),
                		beakOffset: 3
              		       }
            		      );
...
<?	
	...
	while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
		     ...
		     $line['count'] = $i;
                      $user_reports ="<li><small><b>Species</b>: " . addslashes($line['common_name']) . "<br><b>Reported by</b>: " .  addslashes($line['user_name']);
                      if($line['user_friend']) { $user_reports .= " on behalf of "  . addslashes($line['user_friend']); }
                      $user_reports .="<br><b>Observation start date</b>: " .  addslashes($line['obs_start']) . "<br><b>Observation frequency</b>: ";
                      $user_reports .= addslashes($line['frequency']);
		       ...
		       $old_html = $old_html . " " . $user_reports;
                      ...
                      print "var final_content = '" . $old_html . "';";
                     print "\n";
		    ...
		    $user_reports = "<h4>" . addslashes($line['location_name']) . ", " . addslashes($line['city']) . ", " . addslashes($line['state']) . "</h4><ul class=ticker><li><small>";
                    
                    $user_reports .="<b>Species</b>: " . addslashes($line['common_name']) . "<br><b>Reported by</b>: " .  addslashes($line['user_name']);
                    if($line['user_friend']) { $user_reports .= " on behalf of "  . addslashes($line['user_friend']); }
                    $user_reports .="<br><b>Observation start date</b>: " .  addslashes($line['obs_start']) . "<br><b>Observation frequency</b>: " . addslashes($line['frequency']);
