...
if($cur_page == 1 ) {  $cur_page_start = 0; } else { $cur_page_start = $cur_page_start + 16; }
$query1 = "$sql AND l1.valid='1' AND l1.deleted='0' order by photo_id DESC LIMIT $cur_page_start , $per_page";
...
while ($line = mysql_fetch_array($result_final)) {
	...
	if ($n==0 or $n==4) { 
        	$n=0 ; 
        	print "</tr><tr>"; 
        	print "<td style='text-align:center;padding-bottom:20px;'> <a href=\"photo.php?id=$photo_id&$get_url\"><img style='border:solid 5px #c0c0c0;' title='' src='image_uploads/metn_" . addslashes($line["photo_filename"]) . "'></a> <br>";
		//print "<small>" .  stripslashes($line["photo_caption"]) . "</small>";
        	print "</td>";
	} else {
        	print "<td style='text-align:center;padding-bottom:20px;'> <a href=\"photo.php?id=$photo_id&$get_url\"><img style='border:solid 5px #c0c0c0;' title=''";
		print 'src="image_uploads/metn_' .   urlencode(addslashes($line["photo_filename"])) . '"></a><br>';
		//print "<small>" . stripslashes($line["photo_caption"]) . "</small>"; 
        	print "</td>";
	}
